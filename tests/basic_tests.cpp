...
TEST_CASE( "Is Valid Number Sequence", "[Rummikub]" ) {
    ...
    PieceGroup gr;
    CHECK( isValidNumberSequence(gr) == false); // size == 0

    gr.push_back( Piece(BLACK,5) );
    CHECK(  isValidNumberSequence(gr) == false); // size == 1

    gr.push_back( Piece(BLACK,6) );
    CHECK(  isValidNumberSequence(gr) == false); // size == 2

    gr.push_back( Piece(BLACK,7) );
    CHECK(  isValidNumberSequence(gr) == true); // size == 3
    ...
    CHECK( isValidNumberSequence(gr) == true);
    ...
    CHECK( isValidNumberSequence(gr) == false); // two blue, one red
    ...
    gr.push_back( Piece(BLUE,6) );
    CHECK( isValidNumberSequence(gr) == false); // 6 repeated twice

    gr.clear();
    gr.push_back( Piece(BLUE,5) );
    gr.push_back( Piece(BLUE,4) );
    gr.push_back( Piece(BLUE,5) );
    CHECK( isValidNumberSequence(gr) == false); // 5 repeated twice

    gr.clear();
    gr.push_back( Piece(BLUE,2) );
    gr.push_back( Piece(BLUE,4) );
    gr.push_back( Piece(BLUE,5) );
    CHECK( isValidNumberSequence(gr) == false); // missing 3
    ...
    gr.push_back( Jolly );
    gr.push_back( Piece(YELLOW,7) );
    CHECK( isValidNumberSequence(gr) == false); // missing 5 or 6
}
